...
class DatabaseProvider with ChangeNotifier {
  ...
  // in-app memory for holding the Expense exp_cat temporarily
  List<ExpenseCategory> _exp_cat = [];
  List<ExpenseCategory> get exp_cat => _exp_cat;
  ...
  Future<void> _createDb(Database db, int version) async {
    ...
    await db.transaction((txn) async {
      ...
      // insert the initial exp_cat.
      // this will add all the exp_cat to category table and initialize the 'entries' with 0 and 'totalAmount' to 0.0
  ...
  // method to fetch exp_cat

  Future<List<ExpenseCategory>> fetchexp_cat() async {
    // get the database
    ...
    return await db.transaction((txn) async {
      return await txn.query(cTable).then((data) {
        ...
        // set the value of 'exp_cat' to 'nList'
        _exp_cat = nList;
        // return the '_exp_cat'
        return _exp_cat;
      });
  ...
  Future<void> updateCategory(
    ...
    await db.transaction((txn) async {
      await txn
        ...
        var file = _exp_cat.firstWhere((element) => element.title == category);
  ...
  ExpenseCategory findCategory(String title) {
    return _exp_cat.firstWhere((element) => element.title == title);
  }
  ...
  double getHighestExpenseThisWeek() {
  ...
  Expense? get highestExpense {
    ...
    // Sort the list in descending order of amount
    _expenses.sort((a, b) => b.amount.compareTo(a.amount));
    // Retrieve the first element
    return _expenses.first;
  }
}
